...
## 10. CE Pipeline（Phase 70–80：目前較自動化的 workflow）

CE（Compound Engineering）workflow 是目前**較自動化**的一條流程，並在 Phase 78 收斂成一鍵 **Run CE Pipeline**（前端串接既有 runner endpoint，不新增大型 backend）：

```txt
Run CE Pipeline
  → 自動 CE Readonly（Brainstorm / Plan / Audit，唯讀）
  → ⏸ 等使用者 Confirm Work
  → 自動 CE Work（改檔 + verification）→ 自動 CE Review（唯讀）
  → ⏸ 等使用者 Confirm Commit
  → 自動 commit checkpoint（先 verification，通過才 git add tracked + git commit）
  → 自動 Compound Notes + 自動保存 AI Workflow
  →（可選）自動 / 手動 Export CE Artifacts
```

仍維持「半自動、安全優先」原則（與第 6 / 7 節一致）：

- **仍保留兩個人工確認點**：**Work 前**（會改檔）與 **Commit 前**（會建立 commit）。其餘步驟自動接續。
- **不自動 push、不動 remote**：Pipeline 只做到本機 `git commit`。
- **Review needs_fix 不自動修**：Pipeline 停下提示，需手動進 CE Fix Work。
- **completed workflow 防誤跑**（Phase 79B）：已完成的任務 disable Run CE Pipeline，避免再跑到 `nothing_to_commit`。
- **Advanced manual controls 保留為 fallback**（Phase 80）：舊的逐步手動 CE Readonly / Work / Review / Fix / Commit checkpoint 收進預設收合的折疊區，供 Pipeline 失敗或需單獨重跑某階段時使用，非日常主流程。
