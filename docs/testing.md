...
- 由 `e2e/sdd-flow.spec.ts` 與 `e2e/local-runner.spec.ts` 組成（測試數量隨 phase 持續增加，以實際執行輸出為準；CE workflow / CE Pipeline 測試重點見 7.6）。
- 涵蓋範圍：
...
### 7.6 CE workflow / CE Pipeline 測試重點（Phase 70–80）

CE（Compound Engineering）workflow 與一鍵 CE Pipeline 的測試分層如下；指令不變：`npx tsc --noEmit`、`pnpm test:run`、`pnpm test:e2e`，target project 端另有 `npm run verify:local`。

- **Phase 70–77F**（runner 函式層 + UI）：`/ce-readonly-workflow`、`/ce-work`、`/ce-review`、`/ce-fix-work`、`/ce-commit-checkpoint`、`/export-ce-artifacts` 的 gate / parser / 安全邊界；含 Phase 77E 的 verification stdout「完整擷取、不被 pipe flush 截斷」回歸保護，與 Phase 77F 的 commit checkpoint（verification 通過才 commit、只加 tracked、不 push、detached HEAD 可 commit）。
- **Phase 78（CE Pipeline orchestration）**：前端一鍵串接既有 endpoint。重點測：按 Run CE Pipeline 自動跑 Readonly → 停在 Work 確認（不自動 Work）；Confirm Work → 自動 Work + Review；Review needs_fix 停下不自動修；Review passed → 停在 Commit 確認；Confirm Commit → 自動 Compound + Save；Cancel 後不續跑；auto export 預設關閉、勾選才自動匯出。
- **Phase 79B（completed 防誤跑 + wide layout）**：已完成 workflow（commit + Review passed + Compound）時 **「Run CE Pipeline」disabled**，不會誤跑到 `nothing_to_commit`；desktop（≥1100px）雙欄版面、右側 summary panel 存在，<1100px fallback 單欄。
- **Phase 80（UI cleanup）**：主畫面以 Run CE Pipeline 為主；舊手動流程收進 **Advanced manual controls**、欄位 accordion 收進 **Workflow details**，兩者預設收合；E2E 以 `openAdvanced` / `openWorkflowDetails` helper 先展開折疊區再操作既有手動測試。
- 全部仍以 **mock runner 或受控 fake project / 暫存腳本**驅動，不呼叫真實 Claude；CE Pipeline 在測試中亦**不會 push**。
